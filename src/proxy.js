...
export async function proxyRequest(request, targetUrl, corsHeaders = {}) {
  try {
    // Use hardcoded MCP server URL instead of the one from the environment
    const mcpServerUrl = "https://mcp.canary-orion.keboola.dev";
    
    // Normalize target URL (ensure it ends with a slash if it's a base URL)
    const normalizedTargetUrl = mcpServerUrl.endsWith('/') ? mcpServerUrl : `${mcpServerUrl}/`
